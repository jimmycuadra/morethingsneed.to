class Comment < ActiveRecord::Base
  ...
  def no_recent_comment_from_ip
    errors.add(:base, "You're awfully eager to make everyone listen to you. At least wait a single minute before babbling again.") if Comment.where(['created_at > ? AND ip = ?', Time.new.ago(60).in_time_zone, ip]).count > 0
  end
